...
import { 
  ...
  Mail, Calendar, Truck, RefreshCw, Activity,
  Cpu as Processor
} from 'lucide-react';
...
export default function MachineOrders() {
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id) => {
      if (!window.confirm('Confirm permanent archival of this technical asset?')) return;
    ...
    <div className="space-y-8">
        ...
        <div>
           <div className="flex items-center gap-2 mb-2">
              <span className="p-1.5 rounded-lg bg-brand-primary/5 text-brand-primary border border-brand-primary/10">
                 <Processor className="w-4 h-4" />
              </span>
              <span className="text-[10px] font-bold text-brand-secondary uppercase tracking-[0.3em]">Technical Operations</span>
           </div>
           <h1 className="text-4xl font-bold text-brand-primary font-display tracking-tight">Machine Assets</h1>
           <p className="text-brand-text/50 text-sm font-medium mt-1">Laser, CNC, and high-end fabrication coordination panel</p>
        </div>
        <div className="flex items-center gap-3">
           <div className="bg-white px-5 py-3 rounded-2xl border border-brand-primary/10 shadow-sm flex items-center gap-4">
              <div className="flex items-center gap-2">
                 ...
                 <span className="text-[10px] font-bold text-brand-primary uppercase tracking-widest">Production Link</span>
              </div>
      ...
      <div className="flex flex-col lg:flex-row lg:items-center gap-4 bg-white/50 p-2 rounded-2xl border border-brand-primary/5 backdrop-blur-sm">
        <div className="relative flex-1">
           ...
           <input
             type="text"
             placeholder="Search technical manifests or principal IDs..."
             value={search}
      ...
      <div className="card overflow-hidden">
            ...
            <div key={order.id} className="p-8 space-y-6 group hover:bg-brand-surface/30 transition-colors">
              ...
              <div className="flex items-center gap-4 p-5 bg-white rounded-2xl border border-brand-primary/5 shadow-sm">
                 ...
                 <div>
                    ...
                    <p className="text-[10px] text-brand-text/40 font-medium mt-1.5">{order.productName || 'Bespoke Fabrication'}</p>
                 </div>
              </div>
        ...
        <div className="hidden lg:block overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr>
                {['Asset ID', 'Principal', 'Technical Profile', 'State', 'Valuation', 'Action'].map(h => (
                  ...
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-[9px] text-brand-text/40 font-bold uppercase tracking-widest mt-1">{order.productName || 'Custom Specification'}</span>
                      </div>
                    </div>
                  </td>
                  <td className="px-8 py-6">
