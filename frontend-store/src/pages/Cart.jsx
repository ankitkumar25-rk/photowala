  ...
  return (
    <div className={`flex gap-4 p-5 transition-opacity ${removing ? 'opacity-30' : ''}`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-brand-primary font-bold mt-1">â‚¹{Number(item.price).toFixed(2)}<span className="text-xs font-normal text-gray-400 ml-1">each</span></p>
      ...
      <div className="text-right shrink-0 hidden sm:block">
        <p className="font-bold text-xl text-gray-900">â‚¹{lineTotal}</p>
        {item.quantity > 1 && (
          <p className="text-xs text-gray-400 mt-0.5">{item.quantity} Ã— â‚¹{Number(item.price).toFixed(2)}</p>
        )}
      </div>
    </div>
  );
          ...
          <div className="card p-16 text-center max-w-lg mx-auto">
            ...
            <div className="mt-10 pt-8 border-t border-cream-200">
              ...
              <div className="flex flex-wrap gap-2 justify-center">
                {[
                  ['ðŸ† Trophies', '/categories/trophies'],
                  ['ðŸŽ Corporate Gifts', '/categories/corporate-gifts'],
                  ['ðŸ’Ž Momentos', '/categories/momentos'],
                  ['ðŸª„ Personalized Gifts', '/products'],
                ].map(([label, to]) => (
            ...
            <div className="lg:col-span-3 card overflow-hidden">
                ...
                <div className="px-5 pt-4 pb-3 bg-brand-surface border-b border-brand-secondary">
                  <div className="flex items-center justify-between text-sm mb-2">
                    <span className="font-semibold text-brand-secondary flex items-center gap-1.5">
                      <Truck className="w-4 h-4" /> Add â‚¹{freeShipRemaining.toFixed(0)} more for FREE shipping!
                    </span>
              ...
              {shipping === 0 && (
                <div className="px-5 py-3 bg-green-50 border-b border-green-100 flex items-center gap-2 text-sm font-semibold text-green-700">
                  <Tag className="w-4 h-4" /> ðŸŽ‰ You've unlocked FREE shipping!
                </div>
              )}
            ...
            <div className="lg:col-span-2 space-y-4 sticky top-24">
              <div className="card p-6">
                ...
                {shipping > 0 && (
                  <div className="mt-4 p-3 bg-brand-surface rounded-xl text-xs text-brand-secondary font-medium flex items-center gap-2 border border-brand-secondary">
                    ...
                    Add â‚¹{freeShipRemaining.toFixed(0)} more to get FREE shipping on this order
                  </div>
                )}
              ...
              <div className="card p-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {[
                    ['ðŸŒ¾ Grains', '/categories/grains-cereals'],
                    ['ðŸ«’ Oils', '/categories/cold-pressed-oils'],
                    ['ðŸŒ¶ï¸ Spices', '/categories/spices-masalas'],
                  ].map(([label, to]) => (
