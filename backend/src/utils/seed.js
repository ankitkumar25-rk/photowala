...
async function main() {
  ...
  for (const cat of categories) {
    const created = await prisma.category.upsert({
      where: { name: cat.name }, // `name` is the key since it matches the taxonomy exact string
  ...
  for (const p of products) {
    ...
    await prisma.product.upsert({
      ...
      update: {
        price: data.price,
        stock: data.stock,
        categoryId: createdCategories[categorySlug] // in case category changed
      },
      create: {
        ...data,
